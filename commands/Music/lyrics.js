...
class Lyrics extends Command {
  constructor(client) {
    super(client, {
      name: "lyrics",
      description: "Acha a letra da música.",
      aliases: ["ly", "letra"],
      usage: ["letra <música>"]
    });
  }
  ...
  async execute(message, args, Discord) {
    ...
    if (!title)
      return message.channel.send(
        "❌ | Ow, você tem que especificar um termo de pesquisa."
      );
    else {
      ...
      fetch(`https://some-random-api.glitch.me/lyrics?title=${search}`)
        .then(res => res.json())
        .then(lyrics => {
          if (lyrics.error) return message.channel.send("❌ | Não encontrado!");
