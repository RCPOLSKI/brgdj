...
class Music {
  ...
  async handleVideo(video, message, voiceChannel, playlist = false, skip) {
    ...
    if (!serverQueue) {
      ...
      try {
        ...
        return message.channel.send(`😡 | ERRO: \`\`\`${error.message}\`\`\``);
      }
    } else {
      ...
      if (playlist) return;
        ...
        const embed = new Discord.MessageEmbed()
          .setAuthor("Música adicionada!", message.guild.iconURL({ dynamic: true }))
          .setColor("#FFFF00")
          .setDescription(`**[${song.title}](${song.url})**`)
          .setFooter(
            `Requisitada por ${song.requestedBy.tag}`,
  ...
  async play(guild, song, skip = 0, first=false) {
    ...
    if (!guild.me.voice.channel) {
      ...
      return serverQueue.textChannel.send("A música acabou!");
    }
    if (!song) {
      this.client.queue.delete(guild.id);
      serverQueue.voiceChannel.leave();
      return serverQueue.textChannel.send("O player de música parou!");
    }
